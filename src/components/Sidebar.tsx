...
export default function Sidebar({ currentView, onViewChange, isOpen = false, onClose }: SidebarProps) {
  ...
  return (
    <>
      {/* Mobile backdrop overlay — only shown while the drawer is open on small screens */}
      {isOpen && (
        <div
          onClick={onClose}
      ...
      <aside
        id="side-bar-nav"
        className={`fixed left-0 top-0 h-screen w-[240px] bg-white border-r border-[#c6c6cd] flex flex-col p-4 z-50 shadow-sm transition-transform duration-200 ${
          isOpen ? "translate-x-0" : "-translate-x-full"
        } md:translate-x-0`}
      >
        {/* Brand Logo */}
        <div className="mb-8 px-2 flex items-center gap-3">
          <div className="w-10 h-10 bg-black flex items-center justify-center rounded-lg shadow-sm shrink-0">
          ...
          <button
            onClick={onClose}
            className="ml-auto p-1.5 text-[#45464d] hover:text-black rounded-lg hover:bg-[#f2f4f6] md:hidden"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
